...
export async function getSriptUploadInfo(key, script) {
    let hexData;
    if (!isEmptyObj(key)) {
        const encryptResult = await fileEncrypt(key, script);
    ...
    const hexName = stringToHex(script.name);    
    return {hexName, hexData};
}
